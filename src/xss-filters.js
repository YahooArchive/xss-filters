...
exports._getPrivFilters = function () {
    ...
    var URL_ABS = /^[\x00-\x20]*(?:https?:|ftp:|[\/\\]{2})/i,
        URL_REL = /^[\x00-\x20]*(?![a-z][a-z0-9+-.]*:|[\/\\]{2})/i;

    function arrayRegExpFilter_(list, re) {
        for (var r = [], n = list.length, i = 0; i < n; i++) {
            re.test(p[i]) && r.push(protocolList[i]);
        }
    }

    // default is to allow only absoluteUrl with the stricter and faster scheme
    // default is to skip html decoding
    function yuwlFactory(allowRelPath, requireHtmlDecode, hostList, protocolList, inclRelProtocol) {

        var urlAbs = URL_ABS, i = 0, n;

        if (protocolList) {
            protocolList = arrayRegExpFilter_(protocolList, /^[a-z][a-z0-9+-.]*:$/i);
            inclRelProtocol && protocolList.push('[\\/\\\\]{2}');
        }

        if (hostList) {
            hostList = arrayRegExpFilter_(hostList, /[a-z0-9.-]+/i).join('|');

            urlAbs = new RegExp('[\x00-\x20]*(?:' + 
                protocolList ? protocolList.join('|') : 'https:' +
                ')', 'i');
        

        }

        protocolList && (urlAbs = new RegExp('[\x00-\x20]*(?:' + protocolList.join('|') + ')', 'i'););
            
        return function(url) {
            requireHtmlDecode && (url = htmlDecode(url));

            return (urlAbs.test(url) || (allowRelPath && URL_REL.test(url))) ? url : null;
        }




        var urlAbs = !hostList && !protocolList ? URL_ABS :
    ...
    function cssUrl(s) {
        ...
        return URI_BLACKLIST_PROTOCOLS[getProtocol(s, true)] ? '##' + s : s;
    }
